...
using EsoCPU.Instructions.Basic;

namespace EsoCPU.Tests;
...
public class UnitTest
{
    ...
    [TestMethod]
    public void CPUSubleq()
    {
        var cpu = new CPU(new Memory(16));
        cpu.Definitions.AddRange(new IDefinition[] { new Instructions.Subleq.LBL(), new Instructions.Subleq.SUBLEQ() });
        var result = cpu.Parse(new string[] { "LBL 0", "SUBLEQ #0 1 0" });
        Assert.IsTrue(result.Success);
        for (int i = 0; i <= 3; i++) Assert.IsTrue(cpu.Step().Success);
        Assert.IsTrue(cpu.Memory.GetAt(0, out var value));
    ...
    {
        var cpu = new CPU(new Memory(16));
        cpu.Definitions.AddRange(new IDefinition[] { new Instructions.P1eq.LBL(), new Instructions.P1eq.P1EQ() });
        var result = cpu.Parse(new string[] { "LBL 0", "P1EQ #0 1 0" });
        Assert.IsTrue(result.Success);
        for (int i = 0; i <= 2; i++) Assert.IsTrue(cpu.Step().Success);
        Assert.IsTrue(cpu.Memory.GetAt(0, out var value));
        Assert.AreEqual(2, value);
    ...
    [TestMethod]
    public void CPUFlipbit()
    {
        var cpu = new CPU(new Memory(16));
        cpu.Definitions.AddRange(new IDefinition[] { new Instructions.Flipbit.LBL(), new Instructions.Flipbit.FLIPBIT() });
        var result = cpu.Parse(new string[] { "LBL 0", "FLIPBIT #0 0" });
        Assert.IsTrue(result.Success);
        for (int i = 0; i <= 3; i++) Assert.IsTrue(cpu.Step().Success);
        Assert.IsTrue(cpu.Memory.GetAt(0, out var value));
